    ...
    describe '#sorry_try_again' do
      context 'with a single digit length for that state' do
        it "says 'sorry, try again...'" do
          digit_lengths = [16]
          desired_message = "Sorry! That number doesn't look right. Please reply with your 16-digit EBT card number or ABOUT for more information."
      ...
      context 'with multiple possible digit lengths in the state' do
        it "says 'sorry...' with multiple digits" do
          digit_lengths = [16, 19]
          desired_message = "Sorry! That number doesn't look right. Please reply with your 16- or 19-digit EBT card number or ABOUT for more information."
          response = mg.sorry_try_again(digit_lengths)
          expect(response).to eq(desired_message)
        end
      end
      ...
      context 'with no argument passed in' do
        it "says 'sorry, try again...'" do
          desired_message = "Sorry! That number doesn't look right. Please reply with your EBT card number or ABOUT for more information."
