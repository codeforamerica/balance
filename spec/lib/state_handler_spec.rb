...
describe StateHandler::AK do
  describe 'balance transcription processing' do
    ...
    context 'for Yupik' do
      let(:language) { :yupik }

      context 'with transcription containing balance variation 1' do
        it 'sends response with balance amounts' do
          reply_for_user = subject.transcribe_balance_response(successful_transcription_1, language)
          expect(reply_for_user).to eq("Waqaa! Food stamp-an $123.45 amllertauq.")
        end
      end
      ...
      context 'with EBT card not found in system' do
        it 'sends EBT-not-found message' do
          reply_for_user = subject.transcribe_balance_response(transcription_ebt_not_found, language)
          expect(reply_for_user).to eq("Picaqenritamken! Tauna alangqayugnarquq. Kiik cali card-avet number-aari nasvakegki.")
        end
      end
      ...
      context 'with a failed (nil) transcription' do
        it 'sends EBT-not-found message' do
          reply_for_user = subject.transcribe_balance_response(failed_transcription, language)
          expect(reply_for_user).to eq("Picaqenritamken! EBT System-aaq kiuksaituq. Kiik cali EBT Card-avet number-aari text-arniaten ataataacuarqu.")
        end
      end
    end

    context 'for Russian' do
      let(:language) { :russian }

      context 'with transcription containing balance variation 1' do
        it 'sends response with balance amounts' do
          reply_for_user = subject.transcribe_balance_response(successful_transcription_1, language)
          expect(reply_for_user).to eq("Здравствуйте, Ваш баланс по фуд стемпам $123.45.")
        end
      end
      ...
      context 'with EBT card not found in system' do
        it 'sends EBT-not-found message' do
          reply_for_user = subject.transcribe_balance_response(transcription_ebt_not_found, language)
          expect(reply_for_user).to eq("Извините, Ваш номер карточки не найден. Пожалуйста, повторите Ваш номер еще раз. (Сейчас этот сервис работает только в Калифорнии.)")
        end
      end
      ...
      context 'with a failed (nil) transcription' do
        it 'sends EBT-not-found message' do
          reply_for_user = subject.transcribe_balance_response(failed_transcription, language)
          expect(reply_for_user).to eq("Пожалуйста, извините. Мы не можем сейчас соединиться с системой карточек EBT. Пoжалуйстa, введите Ваш номер карточки опять через несколько минут.")
        end
      end
    end
  end
...
describe StateHandler::CA do
  ...
  it 'gives correct button sequence (ebt # with pauses between digits and pound at end)' do
    ...
    expect(desired_sequence).to eq("wwwwwwwwwwwwwwww1wwwwwwwwwwwwww1ww1ww1ww1ww2ww2ww2ww2ww#wwww")
  end
    ...
    context 'for English' do
      ...
      context 'with transcription containing balance variation 2' do
        it 'sends response with balance amounts' do
          ...
          expect(reply_for_user).to eq("Hi! Your food stamp balance is $123.11.")
        end
      end
      ...
      context 'with transcription containing balance variation 3' do
        it 'sends response with balance amounts' do
          ...
          expect(reply_for_user).to eq("Hi! Your food stamp balance is $156.89.")
        end
      end
      ...
      context 'with English language (not number) dollar amounts' do
        it 'sends a numerical value back to the user' do
          ...
          expect(reply_for_user).to eq("Hi! Your food stamp balance is $6.25.")
        end
      end
      ...
      context 'with a transcription with extraneous periods' do
        it 'sends response with balance amounts without extra periods' do
          ...
          expect(reply_for_user).to eq("Hi! Your food stamp balance is $9.11.")
        end
      end
      ...
      context 'with zero dollar values in words' do
        it 'correctly parses the zeroes as values' do
          ...
          expect(reply_for_user).to eq("Hi! Your food stamp balance is $0.00.")
    ...
    context 'for Spanish' do
      ...
      context 'with transcription containing balance variation 2' do
        it 'sends response with balance amounts' do
          ...
          expect(reply_for_user).to eq("Hola! El saldo de su cuenta de estampillas para comida es $123.11.")
        end
      end
      ...
      context 'with transcription containing balance variation 3' do
        it 'sends response with balance amounts' do
          ...
          expect(reply_for_user).to eq("Hola! El saldo de su cuenta de estampillas para comida es $156.89.")
        end
      end
